    ...
    public void GiveOpponentCashChanceCard(Player player, Board board, GUIV2 gui) {
        ...
        while (true) {
            try {
                ...
                if (choice <= board.getPlayers().size() && choice > 0 && !board.getPlayers().get(choice - 1).equals(player)) {
                    System.out.println(
    ...
    public void GiveOpponentCityChanceCard(Player player, Board board, GUIV2 gui) {
        ...
        while (true) {
            try {
                ...
                choicePlayer = gui.getUserInput();
                if (choicePlayer <= board.getPlayers().size() && choicePlayer > 0 && !board.getPlayers().get(choicePlayer - 1).equals(player)) {
                    System.out.println(
